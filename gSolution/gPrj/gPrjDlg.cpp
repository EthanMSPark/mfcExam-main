...
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Text(pDX, IDC_EDIT4_NUM, m_nNUM);
}
...
BEGIN_MESSAGE_MAP(CgPrjDlg, CDialogEx)
	...
	ON_EN_CHANGE(IDC_EDIT4_NUM, &CgPrjDlg::OnEnChangeEdit1)
END_MESSAGE_MAP()
...
void CgPrjDlg::moveRect()
{
	...
	//drawCircle(fm, nSttX, nSttY, m_nNUM, 0xff);
	drawCircle(fm, nSttX++, nSttY++, m_nNUM, nGray);
...
BOOL CgPrjDlg::validImgPos(int x, int y)
{
	int nWidth = m_image.GetWidth();
	int nHeight = m_image.GetHeight();
	CRect rect(0, 0, nWidth, nHeight);
...
void CgPrjDlg::drawCircle(unsigned char* fm, int x, int y, int m_nNUM, int nGray)
{
	int nCenterX = x + m_nNUM;
	int nCenterY = y + m_nNUM;
	int nPitch = m_image.GetPitch();


	for (int j = y; j < y + m_nNUM * 2; j++) {
		for (int i = x; i < x + m_nNUM * 2; i++) {
			if (isInCircle(i, j, nCenterX, nCenterY, m_nNUM))
				fm[j*nPitch + i] = nGray;
		}
	}
}

bool CgPrjDlg::isInCircle(int i, int j, int nCenterX, int nCenterY, int m_nNUM)
{
	bool bRet = false;
